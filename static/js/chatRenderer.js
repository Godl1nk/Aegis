...
import { contextPercentLabel } from './contextUsage.js';
...
export function buildImageBubble(imageUrl, prompt, model, size, quality, imageId) {
  ...
  dlBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    _downloadGeneratedImage(safeImageUrl, prompt, dlBtn);
  });
  ...
  editBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      editorMod.openEditor(safeImageUrl, null, null, label);
    } catch (err) {
...
export function displayMetrics(messageElement, metrics) {
  ...
  if (ctxPct !== undefined && ctxPct > 0) {
    ...
    </svg><span class="ctx-ring-pct">${contextPercentLabel(ctxPct)}</span>`;
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Aegis' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
