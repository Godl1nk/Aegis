...
export function applyModelContextLength(endpointUrl, model, length, selectionVerified = false) {
  if (!selectionVerified && !sameContextSelection(selection, endpointUrl, model)) return;
  if (length == null) {
    snapshot = null;
    revision++;
    render();
