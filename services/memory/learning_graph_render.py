"""Renderer for the learning timeline (learned skills + memories).

Ported verbatim from Hermes (``agent/learning_graph_render.py``); only the
memory-card lookup keys by stable card id instead of positional index, since
Aegis memory entries carry UUIDs. Renders the ``learning_graph`` payload as a
timeline bar chart — date rows, proportional skill/memory bars colored by the
day's dominant category, and a cumulative trajectory sparkline — plus
per-slice bucket metadata the Journey panel walks as a tree.
...
def _bucket_rows(buckets: list[_ChartBucket], payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    # Aegis memory cards carry a stable ``memory:<uuid>`` id — key by it
    # directly (Hermes keyed by positional index because file chunks had none).
    memory_lookup = {
        str(card.get("id")): card
        for card in payload.get("memory", []) or []
...
def render_graph(payload: dict[str, Any], *, cols: int = 80, rows: int = 16, reveal: float = 1.0) -> dict[str, Any]:
    ...
    if not nodes:
        placeholder = [["no learning yet — keep using Aegis and it maps out here", STYLE_DIM, 0.7]]
