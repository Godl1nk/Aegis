"""Scheduled-task creation approval tests (enabled explicitly in this suite)."""
import asyncio
import json
...
def _patch_common(monkeypatch, calls):
    ...
    async def _fake_exec(block, *a, **k):
        ...
        if block.tool_type == "manage_tasks":
            return ("manage_tasks", {"output": "created ok", "exit_code": 0})
...
def _run_loop(monkeypatch, texts, session_id="gate-sess", max_rounds=4):
    ...
    gen = al.stream_agent_loop(
        ...
        relevant_tools={"read_file", "manage_tasks", "manage_memory", "edit_file"},
    )
...
READ = "```read_file\n{\"path\": \"x.py\"}\n```"
CREATE_TASK = '```manage_tasks\n{"action":"create","name":"Test","prompt":"test"}\n```'
EDIT_MEMORY = "```manage_memory\nedit\n4ff50468\nUser's internship is at Hengyuan Refining Company Berhad.\n```"
DONE = "All done."


def test_unit_only_task_creation_requires_approval():
    ctx = ToolRunSecurityContext()
    assert ctx.decision_for("read_file", "").allowed
    assert ctx.decision_for("edit_file", "").allowed
    assert ctx.decision_for("manage_memory", "edit\n1\nfact").allowed
    assert not ctx.decision_for("manage_tasks", '{"action":"create"}').allowed
    ctx.observe_tool_result("read_file", {"output": "data", "exit_code": 0}, "")
    assert ctx.external_untrusted_context_seen
    assert ctx.decision_for("grep", "").allowed
    assert ctx.decision_for("edit_file", "").allowed
    assert ctx.decision_for("manage_memory", "edit\n1\nfact").allowed
    assert ctx.decision_for("manage_tasks", '{"action":"edit"}').allowed
    denied = ctx.decision_for("manage_tasks", '{"action":"create"}')
    assert not denied.allowed
    assert "scheduled task" in (denied.reason or "")
    ctx.approval_gate_bypassed = True
    assert ctx.decision_for("manage_tasks", '{"action":"create"}').allowed
...
def test_unit_unknown_tool_does_not_use_task_creation_card():
    ctx = ToolRunSecurityContext()
    ctx.observe_tool_result("web_search", {"output": "hits", "exit_code": 0}, "")
    assert ctx.decision_for("mystery_tool_xyz", "").allowed


def test_task_creation_prompts_and_once_resumes(monkeypatch):
    calls = []
    _patch_common(monkeypatch, calls)
    events = _pump_with_approval(monkeypatch, [CREATE_TASK, DONE], "once")
    assert any(e.get("type") == "approval_request" for e in events), events
    assert calls == ["manage_tasks"], calls


def test_memory_edit_after_external_read_has_no_approval(monkeypatch):
    calls = []
    _patch_common(monkeypatch, calls)
    events = _run_loop(monkeypatch, [READ, EDIT_MEMORY, DONE], session_id="gate-memory")
    assert not any(e.get("type") == "approval_request" for e in events), events
    assert calls == ["read_file", "manage_memory"], calls


def test_session_choice_bypasses_rest_of_chat(monkeypatch):
    calls = []
    _patch_common(monkeypatch, calls)
    events = _pump_with_approval(monkeypatch, [CREATE_TASK, DONE], "session",
                                 session_id="gate-sess-persist")
    assert any(e.get("type") == "approval_request" for e in events)
    # Second creation in the same session uses the explicit chat grant.
    calls.clear()
    events2 = _run_loop(monkeypatch, [CREATE_TASK, DONE], session_id="gate-sess-persist")
    assert not any(e.get("type") == "approval_request" for e in events2), events2
    assert "manage_tasks" in calls, calls
...
def test_headless_fails_closed_without_wait(monkeypatch):
    ...
    import time
    start = time.time()
    events = _run_loop(monkeypatch, [CREATE_TASK, DONE], session_id="gate-sess-headless")
    ...
    assert calls == [], calls  # task creation never executed
    outputs = [e for e in events if e.get("type") == "tool_output"]
    assert any("requires your approval" in json.dumps(o).lower() for o in outputs), events
...
def test_dialog_timeout_denies_fast(monkeypatch):
    ...
    events = _run_loop(monkeypatch, [CREATE_TASK, DONE], session_id="gate-sess-timeout")
    assert any(e.get("type") == "approval_request" for e in events)
    assert calls == [], calls
...
def test_gate_disabled_sends_everything(monkeypatch):
    ...
    events = _run_loop(monkeypatch, [CREATE_TASK, DONE], session_id="gate-sess-off")
    assert not any(e.get("type") == "approval_request" for e in events)
    assert calls == ["manage_tasks"], calls
...
def test_two_gated_blocks_one_session_approval_covers_both(monkeypatch):
    ...
    calls = []
    _patch_common(monkeypatch, calls)
    two_tasks = CREATE_TASK + "\n" + CREATE_TASK
    events = _pump_with_approval(monkeypatch, [two_tasks, DONE], "session",
                                 session_id="gate-sess-multi")
    cards = [e for e in events if e.get("type") == "approval_request"]
    assert len(cards) == 1, cards
    assert calls == ["manage_tasks", "manage_tasks"], calls
