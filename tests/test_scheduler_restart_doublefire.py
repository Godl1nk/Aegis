...
def _drive_scheduler(monkeypatch, pre_start_setup=None):
    ...
    monkeypatch.setattr(sch, "_auto_update_loop", _never)
    ...
    # _auto_update_loop as tasks (stubbed to _never here); filter those out
    # so the test only counts real per-poll task dispatches.
