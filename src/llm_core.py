...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Aegis' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: Aegis carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): Aegis carries multimodal user content as an
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        ...
        h.setdefault("X-OpenRouter-Title", "Aegis")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Aegis-only metadata before sending messages to providers.
