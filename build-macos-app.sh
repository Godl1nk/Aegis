#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Aegis.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Aegis.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Aegis.dmg   — drag-to-Applications disk image (the downloadable).
#
...
if [ -f "$REPO_DIR/docs/odysseus.jpg" ] && command -v sips >/dev/null 2>&1; then
  ...
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/aegis.icns" >/dev/null 2>&1; then
    echo "  icon:        aegis.icns"
  else
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.godl1nk.aegis.launcher</string>
...
#!/bin/bash
# Aegis.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/aegis-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Aegis\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Aegis\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Aegis isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Aegis failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Aegis is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Aegis to Applications)"
